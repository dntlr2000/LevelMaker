...
이 폴더는 R5의 Blueprint 저장·덮어쓰기·비교·미리보기와 SavedBlueprint StageDefinition 생성 흐름을 Unity Editor에서 직접 확인하기 위한 독립 검증 자료입니다.
...
생성 도구는 `Blueprints/Reference`, `Stages/Reference`, `Settings`, `Scenes`만 기준값으로 갱신합니다. 사용자가 만든 자산은 반드시 `Blueprints/Output`과 `Stages/Output`에 저장하세요. 생성 도구를 다시 실행해도 두 Output 폴더의 자산은 삭제하거나 덮어쓰지 않습니다.
...
5. Project 창에서 자산을 우클릭해 `Reimport`한 뒤 메모, seed `12345`, generatorVersion `2`, Recipe/Catalog/Blueprint hash가 유지되는지 확인합니다.
...
8. 검증을 계속하기 전에 Generator의 Stage Definition을 `R5_Procedural_Seed12345`로 되돌리고 `Load Stage Definition`을 실행합니다.

## 4. 저장본 미리보기와 절차 원본 복귀
...
5. `절차 원본으로 복귀`를 눌러 StableV2 seed 12345 결과와 동일 hash로 돌아오는지 확인합니다.
6. 전환을 3회 반복해도 generated root가 하나뿐이고 Console Error가 없어야 합니다.

## 5. SavedBlueprint StageDefinition 생성
...
- 덮어쓰기 확인과 `Ctrl+Z` 복구가 정상입니다.
- 동일·다른 시드·stale 비교 상태가 기준 자산에 맞게 구분됩니다.
- 저장본 미리보기는 저장 seed/hash를 유지하며 절차 원본으로 복귀할 수 있습니다.
